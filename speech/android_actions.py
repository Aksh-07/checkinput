...
class AndroidActions:
    ...
    def compare_input_string(string_input: str, compare_string: str):
        """compare the user input string with the one fetched from database
    ...
    def get_android_db_words(self, table: str, index: int):
        """Fetch all the rows from given table with the given index then find the one that match the user input and
        stores the string in self.words
        ...
        Returns:
            bytes: r[3] if there is only one word in user input, 4th item of row is fetched from database which is a
    ...
    def decode_user_input_for_android_actions(self, index: int, q_t: queue, lock):
        """Decode anp Process user input after getting an array and index from user_input.convert_strings_to_num_array
        (strings) and put results in queue q_t which can be either
    ...
    def check_android_command_status(self, index):
        """add items in intention, query_type, action_type and item list depending on different conditions as mentioned
        in function if entered input is an android action.
        ...
        try:
            ...
            if is_android_action is not None:
                ...
                try:
                    ...
                    is_android_action = is_android_action + "s".encode("utf_8")
                    word_lst.remove(is_android_action)
                finally:
                    pass
    ...
    def generate_android_action_request():
        """creates a list of dictionaries with keys `query_type`, `item_list`, `description`, `action_type` and its
    ...
    def check_android_description_need():
        ...
        if "photos" in item_list or "music" in item_list or "videos" in item_list:
            ...
            logging.info("description not needed")
            return False
